...
def train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, tf_writer, epoch, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        if  batch_idx == len(train_dl) - 1:
            ...
            path1= os.path.join(opt.temps, "clean_image.png")
            torchvision.utils.save_image(inputs_bd, path, normalize=True)
            torchvision.utils.save_image(inputs[:num_bd], path1, normalize=True)
            print(f"num_bd:{num_bd}")
            print(f"bd:{targets_bd}")
            print(f"preds:{torch.argmax(total_preds[:num_bd], dim=1)}")
            print(f"clean:{targets[:num_bd]}")
